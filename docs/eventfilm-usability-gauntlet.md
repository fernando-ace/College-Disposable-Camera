...
- Confusion found: after creation, the event detail page can put during-event actions in the spotlight because the default event time is "happening now," even when the first useful action is still sharing the guest link.

### Scenario B: Club picnic

- Flow tested: create `Club Picnic` -> choose Simple Album as easiest mode -> find QR poster and photo review actions.
- Passed: QR poster and photo review are available from the host flow; the Share tab keeps the guest link and QR poster easy to find.
...
1. Hosts need a plain explanation of Guest link vs QR poster vs Recap.
2. Shared Recap copy was warmer on the page than in the actual share message.
3. Simple Album needed stronger "safe default" positioning in the create flow.
4. Mobile guest upload was clear, but the first upload controls sat lower than ideal.

## Fixes Made

- Added a collapsed "Which link should I use?" helper in the host Share tab.
- Kept the host Share tab focused on guest link, QR poster, and Recap.
...
- Ask one club officer to decide whether they would use guest link, QR poster, or Recap for a small event.
